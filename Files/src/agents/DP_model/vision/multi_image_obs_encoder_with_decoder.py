...
class MultiImageObsEncoderWithDecoder(ModuleAttrMixin):
    ...
    Adds a Transformer-based fusion layer over per-modality tokens.

    This version enforces a *global bottleneck* z ∈ R^{256} (i.e., [256,1]) for each RGB stream
    when `use_global_bottleneck_for_policy=True`, which is recommended when using the Transformer.

    Flags:
      - use_global_bottleneck_for_policy: return z (B,256) per camera to the policy (or transformer).
      ...
      - use_transformer_fusion: if True, fuse modality tokens with Transformer and return fused output.
      - d_model, nhead, num_transformer_layers, dim_feedforward, transformer_dropout: Transformer params.
      - add_cls_token: prepend a learnable [CLS] token.
      - fusion_output_mode: "cls" | "mean" | "concat" — how to pool Transformer outputs.
    """
    ...
    def __init__(self,
                 ...
                 fusion_output_mode: str = "cls",   # "cls" | "mean" | "concat"
                 ):
        super().__init__()

        assert recon_loss in ['l1', 'l2']
        assert fusion_output_mode in ["cls", "mean", "concat"]
        ...
        self.fusion_output_mode = fusion_output_mode
        ...
        # ============================================================
        # Transformer fusion: token projections & encoder
        # ============================================================
        if self.use_transformer_fusion:
            self.all_token_keys = self.rgb_keys + self.low_dim_keys  # fixed token order for reproducibility

            # Per-token projection into d_model (Lazy: no need to know in-dim ahead of time)
            self.token_proj_map = nn.ModuleDict({k: nn.LazyLinear(self.d_model) for k in self.all_token_keys})

            # Learned token-type embeddings (one per key)
            self.token_type_embed = nn.ParameterDict({k: nn.Parameter(torch.zeros(1, 1, self.d_model))
                                                    for k in self.all_token_keys})
            for p in self.token_type_embed.values():
    ...
    def forward(self, obs_dict: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # Keep per-key feature vectors for fusion
        raw_feats_by_key: Dict[str, torch.Tensor] = {}
            ...
            if len(imgs) > 0:
                ...
                for key, conv_feat in zip(self.rgb_keys, conv_per_key):
                    if self.use_spatial_softmax:
                        ...
                        feat_vec = kps.reshape(batch_size, -1)
                    else:
                        if self.use_global_bottleneck_for_policy:
                            z = self._pool_and_project(conv_feat)  # (B,256)
                            feat_vec = z
                        else:
                            feat_vec = conv_feat.reshape(batch_size, -1)
                    features.append(feat_vec)
                    raw_feats_by_key[key] = feat_vec
        else:
            for key in self.rgb_keys:
                img = obs_dict[key]
                if batch_size is None:
        ...
        for key in self.low_dim_keys:
            ...
            raw_feats_by_key[key] = embed

        # ---------- Transformer fusion ----------
        if self.use_transformer_fusion and len(raw_feats_by_key) > 0:
            tokens = []
            for k in self.all_token_keys:
                if k not in raw_feats_by_key:
                    continue
                x_k = raw_feats_by_key[k]             # [B, Dk]
                x_k = self.token_proj_map[k](x_k)     # [B, d_model]
                x_k = x_k.unsqueeze(1)                # [B, 1, d_model]
                x_k = x_k + self.token_type_embed[k]  # [B, 1, d_model]
                tokens.append(x_k)

            if len(tokens) == 0:
                return torch.cat(features, dim=-1) if len(features) > 0 else torch.empty(batch_size, 0, device=self.device)

            X = torch.cat(tokens, dim=1)  # [B, T, d_model]
            if self.add_cls_token:
            ...
            if self.add_cls_token and self.fusion_output_mode == "cls":
                fused = X[:, 0, :]                     # [B, d_model]
            elif self.fusion_output_mode == "mean":
                start = 1 if self.add_cls_token else 0
                fused = X[:, start:, :].mean(dim=1)    # [B, d_model]
            elif self.fusion_output_mode == "concat":
                fused = X.reshape(batch_size, -1)      # [B, T'*d_model]
            else:
                fused = X[:, 0, :] if self.add_cls_token else X.mean(dim=1)

            return fused

        # ---------- Fallback: original concat ----------
        return torch.cat(features, dim=-1) if len(features) > 0 else torch.empty(batch_size, 0, device=self.device)
...
# Smoke test harness
# ------------------------------
# run with: python -m agents.DP_model.vision.multi_image_obs_encoder_with_decoder
if __name__ == '__main__':
    ...
    # Enforce 256-d bottleneck end-to-end + Transformer fusion
    ...
    run_smoke_tests(obs_encoder, shape_meta, name='Transformer-fused (z256 tokens, CLS pool)')
