...
class PushTEnv_illustration(gym.Env):
    ...
    def __init__(self,
        ...
        self.traj_optimal = np.array([
            [0.171875, -0.1953125],
            [0.14044296, -0.22674454],
            [0.11819268, -0.24899482],
            [0.08444265, -0.25975259],
            [0.05042091, -0.26275259],
            [0.00463045, -0.26275239],
            [-0.04305681, -0.26275259],
            [-0.09074414, -0.26275259],
            [-0.14627514, -0.25275259],
            [-0.18611881, -0.24275259],
            [-0.24437987, -0.21010458],
            [-0.28418263, -0.19024585],
            [-0.32255004, -0.18274555],
            [-0.35417462, -0.15147393],
            [-0.37417462, -0.13147393],
            [-0.39638331, -0.10630862]
        ])

        '''Data from robosuite visualization'''
        # self.traj_negative_1 = np.array([[769, 617],
        #     [771, 625],
        #     [768, 633],
        #     [775, 640],
        #     [767, 647],
        ...
        # self.traj_positive_1 = np.array([ [770, 618],
        #     [779, 627],
        #     [774, 636],
        #     [785, 646],
        #     [785, 654],
        ...
        # def normalize_robosuite_traj(traj):
        #     traj = (traj - 400) / 800
        #     return traj
        # self.traj_positive_1 = normalize_robosuite_traj(self.traj_positive_1)
        # self.traj_negative_1 = normalize_robosuite_traj(self.traj_negative_1)
        # self.traj_negative_1 = self.traj_positive_1


        self.traj_positive_1 = np.array([[ 0.16000999, -0.20531834],# PUSHT
            [ 0.11241943, -0.19762031],
            [ 0.13674596, -0.2884809 ],
            [ 0.05113393, -0.23029186],
            [ 0.07340583, -0.26955127],
            [ 0.03459841, -0.28714649],
            [-0.02019151, -0.25211576],
            [-0.09113111, -0.22359   ],
            [-0.17944395, -0.23972759],
            [-0.20013444, -0.21944236],
            [-0.29920441, -0.21563428],
            [-0.2995787 , -0.1879072 ],
            [-0.30132937, -0.14938326],
            [-0.33543266, -0.1286876 ],
            [-0.37066433, -0.14046078],
            [-0.41473484, -0.07825186]])

        # self.traj_positive_2 = np.array([[ 0.12553124, -0.20336566],
        #     [ 0.11974019, -0.21302112],
        #     [ 0.071500579, -0.23403252],
        #     [ 0.05716282, -0.2708623 ],
        #     [ 0.02743209, -0.27758709],
        #     [ -0.0216162, -0.28417884],
        #     [-0.06347961, -0.29413407],
        #     [-0.08455822, -0.26826147],
        #     [-0.1445076 , -0.2694821 ],
        #     [-0.21732213, -0.23476986],
        #     [-0.27575375, -0.25231123],
        #     [-0.28832415, -0.2141536 ],
        #     [-0.33637215, -0.19775664],
        #     [-0.36061261, -0.15676729],
        #     [-0.3895741 , -0.12567183],
        #     [-0.39990717, -0.10459258]])
        self.traj_positive_2 = self.traj_optimal

        self.traj_negative_1 = np.array([  # PUSHT
            [0.171875, -0.1953125],
            [0.171875, -0.2553125],
            [0.16044296, -0.32674454],
            [0.11819268, -0.38899482],
            [0.05444265, -0.39975259],
            [0.00042091, -0.40275259],
            [-0.06305681, -0.40275259],
            [-0.12074414, -0.3875259],
            [-0.19627514, -0.37975259],
            [-0.28611881, -0.36275259],
            [-0.32437987, -0.34010458],
            [-0.38418263, -0.31024585],
            [-0.46255004, -0.28274555],
            [-0.52417462, -0.25147393],
            [-0.58417462, -0.23147393],
            [-0.63638331, -0.20630862]
        ]) 

        # real data from episode 19
        # self.traj_negative_2 = np.array([[ 0.1266501 , -0.19169165],
        # [ 0.09980488, -0.15925205],
        # [ 0.10148015, -0.2427624 ],
        # [ 0.03906759, -0.18596317],
        # [ 0.03139152, -0.18846214],
        # [-0.02432512, -0.23091106],
        # [-0.07213823, -0.20496772],
        # [-0.15755664, -0.16468683],
        # [-0.1983522 , -0.17766784],
        # [-0.23050304, -0.15333107],
        # [-0.27592418, -0.16929634],
        # [-0.28210208, -0.14169323],
        # [-0.287022  , -0.14082648],
        # [-0.31837884, -0.09743666],
        # [-0.29981118, -0.09233328],
        # [-0.3282994 , -0.04629528]])
        ...
        [ 0.03906759, -0.18596317],
        [ 0.03139152, -0.18846214],
        [-0.02432512, -0.21091106],
        [-0.07213823, -0.20496772],
        [-0.15755664, -0.18468683],
        [-0.1783522 , -0.17766784],
        [-0.21050304, -0.16333107],
        [-0.22592418, -0.15929634],
        [-0.24210208, -0.1509323],
        [-0.267022  , -0.14082648],
        [-0.28837884, -0.11743666],
        [-0.29981118, -0.09233328],
        [-0.3282994 , -0.04629528]])
    ...
    def generate_noisy_positive_traj(self, traj_neg, traj_opt):
        # self.traj_negative_1 = self.traj_optimal + np.random.normal(loc=0, scale=0.05, size=self.traj_optimal.shape)
        difference = traj_opt - traj_neg
        gaussian_noise_scale = np.minimum(np.linalg.norm(difference/2.0), 0.025)
        logger.debug('%s %s', "gaussian_noise_scale: ", gaussian_noise_scale)
        traj_positive  = traj_opt + np.random.normal(loc=0.0, scale=gaussian_noise_scale, size=traj_opt.shape)
        return traj_positive
    ...
    def reset(self):
        ...
        state = np.array([
             300,206,
            196- 0* np.cos(np.pi/4), 176+50* np.cos(np.pi/4),
            np.pi* 3/5
            ])
        ...
        #     else:
        #         print("self.agent.position: ", self.agent.position)
    ...
    def step(self, action):
        ...
        logger.debug('%s %s', "action: ", action)
        ...
        if not self.abs_action:
            ...
            if not (np.all((action_ >= -1) & (action_ <= 1))):
                logger.debug("Action is out of bounds!")
                import pdb; pdb.set_trace()
    ...
    def render_with_agent_actions(self, traj_list, action_pair_id=1, mode = "human"):
        ...
        # self._visualize_desiredA(self.traj_optimal, self.traj_positive_1, self.traj_negative_1)
        # self._visualize_traj(self.traj_optimal)

        # self._visualize_traj(self.traj_negative_2)

        if action_pair_id == 1:
            traj_positive = self.traj_positive_1
            traj_negative = self.traj_negative_1
        elif action_pair_id == 2:
            traj_positive = self.traj_positive_2
            traj_negative = self.traj_negative_2
        
        self._visualize_desiredA_outline(self.traj_optimal, traj_positive, traj_negative)

        #TODO visualize a traj numpy array shape of [num_traj, 16, 2]
        for traj in traj_list:
            self._visualize_traj(traj)

        # self._visualize_traj(traj_negative, negative=True, color_map='Blues')
        # self._visualize_traj(traj_positive, positive= True, color_map='Blues')

        # import pdb; pdb.set_trace()
        # self._visualize_traj(traj_positive_2)
        # self._visualize_desiredA(self.traj_optimal, traj_positive_2, self.traj_negative_2)
    ...
    def _get_obs(self):
        ...
        concatenated_obs = np.concatenate((self.previous_obs, expanded_obs))
    ...
    def draw_marker(self, pos, negative=False, positive=False, color=(0, 0, 0) , size=4, width=1):
        # x, y = map(int, pos)
        x, y = pos
    ...
    def _visualize_traj(self, traj, negative=False, positive=False, color_map = 'viridis'):
        # traj is a list of actions
        ...
        path = [self.unnormalize_abs_action(traj[i], action_max=self.eef_pos_max_list[0], action_min=self.eef_pos_min_list[0])
                for i in range(len(traj))]
        # # ## draw line
        # for i in range(len(traj) - 1):
        #     # print("path: ", path)
        #     pygame.draw.line(self.screen, (0, 0, 255), tuple(map(int, path[i])), tuple(map(int, path[i+1])), width=2)  # Red path

        # for i in range(len(traj) ):
        #     pygame.draw.circle(self.screen, marker_color, tuple(map(int, path[i])), marker_radius)
        import matplotlib.cm as cm
        colormap = cm.get_cmap(color_map)  # You can choose other colormaps too
        # colormap = cm.get_cmap('Blues')  # You can choose other colormaps too

        for i in range(1,len(traj)):
            rgba = colormap(i / (len(traj) - 1))  # Normalized index
            rgb = tuple(int(255 * c) for c in rgba[:3])  # Convert to 0–255 RGB
            pygame.draw.circle(self.screen, rgb, tuple(map(int, path[i])), marker_radius)
       
            # color = (240 - 12 * i, 240- 12 * i, 240- 12 * i)
            i_color = i
            if i >= 6 and i <= 10:
                i_color = 5
            color = (16 *i_color, 16 * i_color, 16 * i_color)
            # color = (240 - 5 * i, 240- 5 * i, 240- 5 * i)

            self.draw_marker(tuple(map(int, path[i])), negative = negative, positive = positive, color=color)


    def _visualize_desiredA_outline(self, traj, traj_positive, traj_negative):
        import alphashape
        ...
        path = [self.unnormalize_abs_action(traj[i], action_max=self.eef_pos_max_list[0], action_min=self.eef_pos_min_list[0])
                for i in range(1, len(traj))]
        pos_path = [self.unnormalize_abs_action(traj_positive[i], action_max=self.eef_pos_max_list[0], action_min=self.eef_pos_min_list[0])
                    for i in range(1, len(traj))]
        neg_path = [self.unnormalize_abs_action(traj_negative[i], action_max=self.eef_pos_max_list[0], action_min=self.eef_pos_min_list[0])
                    for i in range(1, len(traj))]

        # 2) Build a list of Shapely circle‐polygons
        circles = []
        for p, n in zip(pos_path, neg_path):
            p = np.array(p)
            n = np.array(n)
            radius = np.linalg.norm(0.5 * (p - n))
            # buffer(resolution=32) gives a smooth circle approximation
            circles.append(Point(p[0], p[1]).buffer(radius, resolution=32))

        # 3) Merge them all
        union_shape = unary_union(circles)

        # 4) Extract exterior coordinates
        outlines = []
        if isinstance(union_shape, Polygon):
            outlines.append(list(union_shape.exterior.coords))
        # elif isinstance(union_shape, MultiPolygon):
        #     for poly in union_shape:
        #         outlines.append(list(poly.exterior.coords))
        elif isinstance(union_shape, MultiPolygon) or getattr(union_shape, "geom_type", None) == "MultiPolygon":
            for poly in union_shape.geoms:
                outlines.append(list(poly.exterior.coords))

        # 5) Draw each outline with pygame
        marker_color = (80, 80, 80)  
        line_width   = 1

        for coords in outlines:
            # convert to list of integer (x,y) tuples
            pts = [(int(x), int(y)) for x, y in coords]
            # Option A: solid polygon outline
            pygame.draw.polygon(self.screen, marker_color, pts, width=line_width)
            # Option B: anti-aliased lines (uncomment if you prefer smoother edges)
            # pygame.draw.aalines(surface, marker_color, closed=True, points=pts)

    def _visualize_desiredA(self, traj, traj_positive, traj_negative):
        # traj is a list of actions
        ## draw the path cadidate 
        ...
        #     pygame.draw.circle(self.screen, marker_color, tuple(map(int, path[i])), marker_radius)
        import matplotlib.cm as cm
        colormap = cm.get_cmap('viridis')  # You can choose other colormaps too

        # Draw colored circles with radius based on the distance
        for i in range(1,len(traj)):
        # for i in [ 5, 9, 14]:
            rgba = colormap(i / (len(traj) - 1))
            rgb = tuple(int(255 * c) for c in rgba[:3])
            
            center = tuple(map(int, pos_path[i]))
            diff = np.array(pos_path[i]) - np.array(neg_path[i])
            radius = int(np.linalg.norm(0.5 * diff))
            # pygame.draw.line(self.screen, rgb, tuple(map(int, pos_path[i])), tuple(map(int, neg_path[i])), width=2)  # Red path
            pygame.draw.circle(self.screen, rgb, center, radius, width=2)
        # for i in range(len(traj)):
        #     rgba = colormap(i / (len(traj) - 1))
        #     rgba = rgba[:3] + (0.5,)
        #     rgb = tuple(int(255 * c) for c in rgba[:3])
        #     alpha = int(255 * rgba[3])  # Usually rgba[3] is 1.0 in colormaps
        ...
        #     circle_surface = pygame.Surface((radius * 2 + 2, radius * 2 + 2), pygame.SRCALPHA)

        #     # Draw circle on this surface
        #     pygame.draw.circle(circle_surface, (*rgb, alpha), (radius + 1, radius + 1), radius)

        #     # Blit this surface to the screen
        #     self.screen.blit(circle_surface, (center[0] - radius - 1, center[1] - radius - 1))


    def _render_frame(self, mode= "human", action_pair_id=1):
        ...
        for shape in self.block.shapes: 
            logger.debug('%s %s', "GLOBAL:", [(v.x, v.y) for v in[self.block.local_to_world(v) for v in shape.get_vertices()]])

       
        ##get local vertices: for shape in self.block.shapes: print("LOCAL:", shape.get_vertices())
        ##get global vertices: for shape in self.block.shapes: print("GLOBAL:", [self.block.local_to_world(v) for v in shape.get_vertices()])
        ...
        # self._visualize_desiredA(self.traj_optimal, traj_positive, traj_negative)
    ...
    def control_policy(self, robot_action):
        ...
        if i_point_list is not None:
            ...
            if self.abs_action:
                teacher_abs_action = self.agent.position 
                teacher_abs_action = self.normalize_abs_action(teacher_abs_action, action_max=self.action_max_list[0], action_min=self.action_min_list[0])
