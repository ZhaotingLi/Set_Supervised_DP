...
import env.robotsuite.tensor_utils as TU
...
class ObservationKeyToModalityDict(dict):
    ...
    def __getitem__(self, item):
        ...
        if item not in self.keys():
            logger.debug(f"ObservationKeyToModalityDict: {item} not found,"
...
def initialize_obs_utils_with_obs_specs(obs_modality_specs):
    ...
    for obs_modality, obs_keys in OBS_MODALITIES_TO_KEYS.items():
        logger.debug("using obs modality: {} with keys: {}".format(obs_modality, obs_keys))
...
class ImageModality2(ImageModality):
    """
    Modality for RGB image observations
    """
    name = "rgb2"
